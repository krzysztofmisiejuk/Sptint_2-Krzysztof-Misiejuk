...
if (editForm) {
	editForm.addEventListener('submit', async (e) => {
		...
		const res = await fetch(
			`http://localhost:5555/users/edit/${editingUserId}`,
			{
				method: 'PUT',
...
function setupEventListeners() {
	...
	if (profileForm) {
		profileForm.addEventListener('submit', async (e) => {
			...
			const res = await fetch(`http://localhost:5555/users/edit`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
	...
	if (buyCarForm) {
		buyCarForm.addEventListener('submit', async (e) => {
			...
			const res = await fetch(`http://localhost:5555/cars/${carId}/buy`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
...
function setupSSE() {
	...
	evtSource.onmessage = (event) => {
		...
		showNotification(
			`SSE: ${msg.event} - Car ID: ${msg.carId}, Buyer ID: ${msg.ownerId}`
		);
	};
}
